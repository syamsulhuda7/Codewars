...
function high(x) {
    ...
    const sumNumericValues = string => string.toLowerCase().split('').reduce((acc, char) => acc + char.charCodeAt(0) - 96, 0);

    const avalue = a.map(sumNumericValues);

    const result = avalue.reduce((acc, val, idx) => val > acc.max ? { max: val, maxIndex: idx } : acc, { max: -Infinity, maxIndex: -1 });
